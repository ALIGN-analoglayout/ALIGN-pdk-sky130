...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if generator_name == 'CAP':
        ...
        for key in mvalues:
            ...
            name_arg = 'W'+str(float(mvalues[key]["W"])*1E+9)
        ...
        if isinstance(size, int):
            for key in mvalues:
                assert int(float(mvalues[device_name]["W"])*1E+9) == int(float(mvalues[key]["W"])*1E+9), f"W should be same for all devices in {name} {mvalues}"
